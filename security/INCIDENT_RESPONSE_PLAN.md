...
| 2026-08-28 | SEV1 | While investigating an `npm audit` finding (risk #7), Claude ran a full `node_modules` copy and, separately, a from-scratch `npm install` directly on both the primary and backup EC2 instances to test a `pdfjs-dist` major-version bump. Neither instance has swap, only 912Mi RAM total, and `/tmp` is `tmpfs` (RAM-backed) — the combined memory pressure made the SSM agent on both instances unresponsive (`ConnectionLost`), and the primary's `adma-api` itself stopped answering requests, taking the live site down for all users (no load balancer — primary serves directly). | EC2-level status checks (reachability/system/EBS) stayed "ok" throughout, ruling out an actual crash. Rebooted both instances via the EC2 console (SSM was unusable to fix this remotely); `adma-api` came back via pm2 `autorestart` and health checks passed again on both. Total downtime on primary bounded by detection-to-reboot time; exact duration not separately measured. | Resolved same day, no data exposure. Process lesson, applied immediately: retested the same upgrade using a disk-backed path (`/home/ec2-user`, not `/tmp`) and `npm install --omit=optional` plus a targeted native-binary install instead of a full reinstall — completed in ~2 minutes with memory headroom to spare. **Going forward: any test/maintenance operation involving a full dependency reinstall, bulk file copy, or native compilation must not run directly on these production instances** — use a disk-backed path at minimum, and prefer a disposable/non-production environment for anything compile-heavy, until the instances have swap or more RAM (see risk register). |
