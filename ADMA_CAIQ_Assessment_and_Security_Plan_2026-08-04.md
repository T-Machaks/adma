...
| 16 | Move off a single EC2 instance | **Fully costed, confirmed blocked on IAM.** Directly tested (not assumed) `ec2:DescribeInstances`/`CreateImage`/`CreateSnapshot`, DLM, and even a CloudWatch-alarm-only auto-recovery approach — all denied, the last one needing `iam:CreateServiceLinkedRole` specifically. See `security/INFRASTRUCTURE_RESILIENCE_OUTLINE.md` for exact console steps plus costs: **option 1 (snapshot + auto-recovery) ~$1–5/month**, **option 2 (warm standby) ~$10–15/month**, **option 3 (full multi-AZ) ~$35–50+/month** — against a ~$10/month baseline today. Recommendation stands: option 1 is the best value, needs ~15 minutes of the account owner's console time, not a project. |
